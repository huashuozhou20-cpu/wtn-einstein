...
### Time Manager Tuning
- Both `runner` and `tournament` accept optional knobs to tune dynamic budgets. Examples:
  ```bash
  python -m einstein_wtn.runner --mode game --red opening-expecti --blue heuristic --tm-safe-cap-frac 0.1 --tm-base-frac 0.9
  python -m einstein_wtn.tournament --games 200 --red opening-expecti --blue heuristic --time-limit-seconds 10 --tm-safe-cap-frac 0.1
  ```
  These flags scale the baseline computed from remaining time divided by estimated moves left and clamp each step to a safe fraction of the remaining clock.
- Presets simplify tuning: `--tm-preset fast` for short clocks and `--tm-preset slow` for long clocks (overrides may still be provided). Example benches:
  ```bash
  python -m einstein_wtn.tournament --games 200 --red opening-expecti --blue heuristic --seed 0 --stats --time-limit-seconds 10 --tm-preset fast
  python -m einstein_wtn.tournament --games 200 --red opening-expecti --blue heuristic --seed 0 --stats --time-limit-seconds 240 --tm-preset slow
  ```
